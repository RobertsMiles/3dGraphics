...
const int VERTICAL_COMPRESSION = 1;
 ...
 * TODO remove
 */
void calculateHeatMap(int x, int y, int z, int xScreen, int yScreen, int screen[HEIGHT_SCR][WIDTH_SCR]) {
	// calculates distance from screen for heatmap
	double distance = sqrt(pow(x, 2) + pow(y, 2) + pow(z, 2));
	int percentageAway = round((distance / RENDER_DISTANCE * 100));
